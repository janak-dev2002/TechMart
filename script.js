...
function basicSearch(x) {
  ...
  r.onreadystatechange = function () {
    if (r.readyState == 4) {
      var t = r.responseText;
      window.location =
        "productListning.php?selected_category=" +
        select +
        "&searchKey=" +
        txt.value;
    }
...
function selectCategory(urlName, Minicid) {
  ...
  r.onreadystatechange = function () {
    if (r.readyState == 4) {
      window.location =
        "selectedCategory.php?ur=" + urlName + "&mid=" + Minicid;
    }
  };

  r.open(
    "GET",
    "selectedCategory.php?ur=" + urlName + "&mid=" + Minicid + "&btnId=" + btnId
  );
  r.send();
}

function selectSubCategory(urlName,Sid){
    var btnId = "1";
    ...
    r.onreadystatechange = function () {
      if (r.readyState == 4) {
        window.location =
          "selectedCategory.php?ur=" + urlName + "&subid=" + Sid;
      }
    };
  
    r.open(
      "GET",
      "selectedCategory.php?ur=" + urlName + "&subid=" + Sid + "&btnId=" + btnId
    );
    r.send();
}

function selectMainCategory(urlName,Sid){
    var btnId = "1";
    ...
    r.onreadystatechange = function () {
      if (r.readyState == 4) {
        window.location =
          "selectedCategory.php?ur=" + urlName + "&mainbid=" + Sid;
      }
    };
  
    r.open(
      "GET",
      "selectedCategory.php?ur=" + urlName + "&mainbid=" + Sid + "&btnId=" + btnId
    );
    r.send();
}
...
function addtoCartInItemView(pid) {
  var buyQty = parseInt(document.getElementById("qty_input").value, 10);

  var qtyElement = document.getElementById("qty_available");
  // Get the text content of the element
    // Get the text content of the element
  var qtyText = qtyElement.textContent || qtyElement.innerText;
  // Extract the numerical value from the text
  var qtyMatch = qtyText.match(/\d+/);
  var qty_available = qtyMatch ? parseInt(qtyMatch[0], 10) : 0;

  if(qty_available >= buyQty){
  ...
  r.onreadystatechange = function () {
    if (r.readyState == 4) {
      ...
      window.location.replace("productView.php?id=" + pid);
  ...
  r.open(
    "GET",
    "addtoCartProccessItemView.php?id=" + pid + "&bqty=" + buyQty,
    true
  );
  r.send();
  
  }else{
    alert("Sorry ! We have only "+qty_available+" Items in stock !");
  }
...
function payNow(id, qty, btn_type, shipping) {
  if (btn_type == 0) {
      ...
      if (r.readyState == 4) {
        ...
        if (t == "1") {
          ...
          var payment = {
            ...
            return_url: "http://localhost/techmart/purchaseHistory.php", // Important
            cancel_url: "http://localhost/techmart/productListning.php", // Important
   ...
   // alert("From Cart");
    ...
    //         apiKey:
    //           "767d0a38388781fc4baa6c4713951ca717a2f5a7e395807e67cf297635bc40df",
    //       });
    //     }
    //   }
    // };
...
function goTobuyThis(pid) {
  var qty = parseInt(document.getElementById("qty_input").value, 10);
  var qtyElement = document.getElementById("qty_available");
