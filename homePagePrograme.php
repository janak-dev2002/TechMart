...
if ($_GET["btn"] == "1") {
    ...
    $product_rs = Database::search("SELECT * FROM `product` ORDER BY `datetime_added` DESC LIMIT 4");
...
?>
        ...
        <div class="col-lg-3 mt-4 mb-4 card bg-light border-0 shadow" style="width: 19rem;">
            ...
            <div class="card-body text-center mt-3">
                ...
                <h2 class="price">Rs. <?php echo number_format($product_data["price"],2); ?></h2>
                <h6>Availability : &nbsp; <?php echo $product_data["qty"]; ?> item(s)</h6>
                <button onclick="addtoCart(<?php echo $product_data['id']; ?>);" class="mt-3 btn btn-outline-dark search-btn " style="width: 220px; height: auto;">
    ...
    $brnHasModal_rs = Database::search("SELECT * FROM `brand_has_model` WHERE `Sub_category_id` = '3'ORDER BY `id` DESC LIMIT 4");

    for ($y = 0; $y < $brnHasModal_rs->num_rows; $y++) {

        $brndHasModal_data = $brnHasModal_rs->fetch_assoc();

        $product_rs = Database::search("SELECT * FROM `product` WHERE `brand_has_model_id` = '" . $brndHasModal_data["id"] . "'");
        $product_num = $product_rs->num_rows;
